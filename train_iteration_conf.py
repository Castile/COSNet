...
def get_arguments():
    ...
    parser.add_argument("--gpus", type=str, default="0", help="choose gpu device.") #使用3号GPU
...
def configure_dataset_init_model(args):
    if args.dataset == 'voc12':
        ...
        args.data_list = './dataset/train_aug.txt'  # Path to the file listing the images in the dataset
        args.ignore_label = 255     #The index of the label to ignore during the training
        args.input_size = '378,378' #Comma-separated string with height and width of images  '473,473'
        ...
        args.batch_size = 4   #Number of images sent to the network in one step, batch_size/num_GPU=2
...
def loss_calc1(pred, label):
    ...
    """
    labels = torch.ge(label, 0.5).float()  # 大于等于： 大于0.5 返回1，  小于 0.5 的返回0
#    
...
#    
    ...
    weight_1 = torch.reciprocal(weight_1)  # 倒数
...
def get_10x_lr_params(model):
    ...
    if torch.cuda.device_count() == 1:
        ...
        b.append(model.main_classifier2.parameters())
    else:
...
def main():
    ...
    configure_dataset_init_model(args)  # 设置参数
    print(args)
    ...
    torch.manual_seed(args.random_seed)  # 设置随机种子， 这句话会使每次运行的随机数都一致
    if args.cuda:
        torch.cuda.manual_seed(args.random_seed) 

    h, w = map(int, args.input_size.split(',')) # 训练图像的大小
    input_size = (h, w)

    cudnn.enabled = True

    print("=====> Building network")
    saved_state_dict = torch.load(args.restore_from)  # 加载状态；保存的状态字典
    model = CoattentionNet(num_classes=args.num_classes)# 加载模型： 共同注意力模型
    #print(model)
    new_params = model.state_dict().copy()  # torch.nn.Module模块中的state_dict变量存放训练过程中需要学习的权重和偏置系数
    ...
    """
    model.train()
    cudnn.benchmark = False  #  设置这个 flag 可以让内置的 cuDNN 的 auto-tuner 自动寻找最适合当前配置的高效算法，来达到优化运行效率的问题。

    if not os.path.exists(args.snapshot_dir):  # 创建保存模型snapshot的文件夹
    ...
    logFileLoc = args.snapshot_dir + args.logFile # 日志文件
