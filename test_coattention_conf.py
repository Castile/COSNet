...
def get_arguments():
    ...
    parser.add_argument("--seq_name", default = 'AN119T')
...
def configure_dataset_model(args):
    if args.dataset == 'voc12':
        ...
        args.maxEpoches = 1 # 1 card: 15, 2 cards: 15 epoches, equal to 30k iterations, max iterations= maxEpoches*len(train_aug)/batch_size_per_gpu'),
        args.data_dir = './dataset/DAVIS-2016'   # 37572 image pairs
        # args.data_list = './DAVIS/test_seqs.txt'  # Path to the file listing the images in the dataset
        args.data_list = './dataset/val_seq.txt'
        args.ignore_label = 255     #The index of the label to ignore during the training
...
def main():
    ...
    for index, batch in enumerate(testloader):
        ...
        for i in range(0,args.sample_range):  
            ...
            # output = model(Variable(target, volatile=True), Variable(search_im, volatile=True)) # 使用cpu
        ...
        first_image = np.array(Image.open(args.data_dir + '/JPEGImages/480p/bear/00000.jpg'))
