...
class PairwiseImg(Dataset):
    ...
    def __init__(self, train=True,
        ...
        if self.seq_name is None: #所有的数据集都参与训练
            with open(os.path.join(db_root_dir, fname + '.txt')) as f:
                ...
                #data_list = np.sort(os.listdir(db_root_dir))
                    for seq in seqs: #所有数据集
                        seq = seq.strip('\n') 
                        images = np.sort(os.listdir(os.path.join(img_root_dir,seq.strip())+'/DUTS-TR-Image/'))#针对某个数据集，比如DUT
            # Initialize the original DAVIS splits for training the parent network
                        images_path = list(map(lambda x: os.path.join((seq +'/DUTS-TR-Image'), x), images))
                        image_list.extend(images_path)
                        # print(images_path)
                        lab = np.sort(os.listdir(os.path.join(img_root_dir,seq.strip())+'/DUTS-TR-Mask'))
                        lab_path = list(map(lambda x: os.path.join((seq +'/DUTS-TR-Mask'),x), lab))
            ...
            labels = [os.path.join( (str(seq_name)+'/DUTS-TR-Mask'), names_img[0])]
