        ...
        if self.seq_name is None: #所有的数据集都参与训练
            with open(os.path.join(db_root_dir, fname + '.txt')) as f:
                ...
                for seq in seqs:                    
                    ...
                    labels = [os.path.join((str(seq_name) + '/saliencymaps'), img_list[0])]
                    # labels = os.listdir(os.path.join(db_root_dir, 'Annotations/480p/blackswan'))

                    labels.extend([None] * (len(img_list) - 1))  # 在labels这个列表后面添加元素None
                    if self.train:
                        img_list = [img_list[0]]
                        labels = [labels[0]]
